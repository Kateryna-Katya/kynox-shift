...
document.addEventListener('DOMContentLoaded', () => {
    ...
    navToggle?.addEventListener('click', () => {
        ...
        // Cambia icona se necessario (opzionale)
    });
    ...
    if (typeof TypeIt !== 'undefined' && document.querySelector('#typeit-terminal')) {
        ...
        .type('<span class="code-comment">// Protocollo Kynox v2.4</span>')
        .break().type('> Collegamento UE... <span class="code-success">[OK]</span>')
        .break().type('> Moduli AI attivi.')
        .break().pause(1000).type('> Efficienza +45%')
        .go();
    }
    ...
    if (form) {
        ...
        form.addEventListener('submit', (e) => {
            ...
            setTimeout(() => {
                ...
                btn.innerHTML = 'Invia Richiesta <i data-lucide="send"></i>';
                lucide.createIcons();
            }, 1500);
        });
    }

    // 9. THREE.JS HERO (Inizializzazione già definita in precedenza)
    if (typeof THREE !== 'undefined') {
        initThreeJS(); 
    }
});

// Nota: La funzione initThreeJS() deve essere inclusa qui o definita globalmente come fatto nell'Etapa 3.